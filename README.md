# Luminous Dynamics Organization Documentation

*"In unity we code, in love we debug, in consciousness we deploy"* ✨

Welcome to the Luminous Dynamics `.github` repository - home to our organization-wide documentation, community health files, and governance framework.

## 🌟 About Luminous Dynamics

Luminous Dynamics is building **Coherence as a Service (CaaS)** - consciousness infrastructure for humanity's awakening based on **Evolving Resonant Co-creationism (ERC)** and the Meta-Principle of Infinite Love.

→ See our [Organization Profile](https://github.com/Luminous-Dynamics)

## 📖 Documentation Index

### Getting Started
- **[GETTING_STARTED.md](./GETTING_STARTED.md)** - Complete newcomer onboarding guide
- **[FAQ.md](./FAQ.md)** - Frequently asked questions
- **[GLOSSARY.md](./GLOSSARY.md)** - Terminology and concepts explained
- **[LEARNING.md](./LEARNING.md)** - Learning paths and educational resources

### Community
- **[CODE_OF_CONDUCT.md](./CODE_OF_CONDUCT.md)** - Community standards and expectations
- **[CONTRIBUTING.md](./CONTRIBUTING.md)** - How to contribute (code and non-code)
- **[CONTRIBUTORS.md](./CONTRIBUTORS.md)** - Contributor recognition and hall of fame
- **[SUPPORT.md](./SUPPORT.md)** - Getting help and support

### Organization
- **[VISION_AND_ROADMAP.md](./VISION_AND_ROADMAP.md)** - Strategic vision and roadmap (2025-2032)
- **[GOVERNANCE.md](./GOVERNANCE.md)** - Governance model and decision-making
- **[SUSTAINABILITY.md](./SUSTAINABILITY.md)** - Funding model and sustainability plan
- **[SECURITY.md](./SECURITY.md)** - Security policy and vulnerability reporting
- **[RESEARCH.md](./RESEARCH.md)** - Research publications and academic contributions

### Repository Management
- **[REPOSITORY_TOPICS.md](./REPOSITORY_TOPICS.md)** - Topic tags (discoverability-focused)
- **[REPOSITORY_TOPICS_ALIGNED.md](./REPOSITORY_TOPICS_ALIGNED.md)** - Topic tags (ERC-focused)
- **[PINNED_REPOSITORIES.md](./PINNED_REPOSITORIES.md)** - Repository curation strategy
- **[IMPROVEMENTS_SUMMARY.md](./IMPROVEMENTS_SUMMARY.md)** - Documentation of all improvements

### GitHub Integration
- **[FUNDING.yml](./FUNDING.yml)** - Sponsorship and support links
- **[workflows/](./workflows/)** - GitHub Actions automation (greetings, stale issues, labels)
- **[ISSUE_TEMPLATE/](./ISSUE_TEMPLATE/)** - Issue templates (bug, feature, question, config)
- **[DISCUSSION_TEMPLATE/](./DISCUSSION_TEMPLATE/)** - Discussion templates (general, proposal, philosophy)
- **[PULL_REQUEST_TEMPLATE.md](./PULL_REQUEST_TEMPLATE.md)** - PR template with ERC alignment

## 🚀 Main Projects

1. **[Luminous Dynamics](https://github.com/Luminous-Dynamics/luminous-dynamics)** - Main consciousness-first computing ecosystem
2. **[Mycelix Protocol](https://github.com/Luminous-Dynamics/Mycelix-Core)** - Byzantine-resistant federated learning
3. **[Kosmic Lab](https://github.com/Luminous-Dynamics/kosmic-lab)** - AI-accelerated consciousness research
4. **[Terra Atlas](https://github.com/Luminous-Dynamics/terra-atlas)** - Live global energy investment platform
5. **[Luminous Nix](https://github.com/Luminous-Dynamics/luminous-nix)** - Natural language for NixOS
6. **[ERC Philosophy](https://github.com/Luminous-Dynamics/evolving-resonant-cocreationism)** - Our philosophical foundation

→ [See all 15+ projects](https://github.com/Luminous-Dynamics)

## 🌐 Quick Links

- **Organization**: [github.com/Luminous-Dynamics](https://github.com/Luminous-Dynamics)
- **Documentation**: [docs.luminousdynamics.org](https://docs.luminousdynamics.org)
- **Developer Portal**: [luminousdynamics.io](https://luminousdynamics.io)
- **Main Website**: [luminousdynamics.org](https://luminousdynamics.org)
- **Support**: [Ko-fi](https://ko-fi.com/luminousdynamics) | [Investment](https://terra-lumina.com/invest)

## 🎯 Quick Start Paths

### For Contributors
1. Read [GETTING_STARTED.md](./GETTING_STARTED.md)
2. Review [CONTRIBUTING.md](./CONTRIBUTING.md)
3. Check [CODE_OF_CONDUCT.md](./CODE_OF_CONDUCT.md)
4. Find a project and start contributing!

### For Users
1. Explore our [projects](https://github.com/Luminous-Dynamics)
2. Try [Terra Atlas](https://atlas.luminousdynamics.io) or [Infin Love](https://infin.love)
3. Check [FAQ.md](./FAQ.md) for common questions
4. Join [Discussions](https://github.com/orgs/Luminous-Dynamics/discussions)

### For Researchers
1. Review [Vision & Roadmap](./VISION_AND_ROADMAP.md)
2. Explore [Kosmic Lab](https://github.com/Luminous-Dynamics/kosmic-lab)
3. Study [Mycelix Protocol](https://github.com/Luminous-Dynamics/Mycelix-Core)
4. Check [ERC Philosophy](https://github.com/Luminous-Dynamics/evolving-resonant-cocreationism)

## 📁 Repository Structure

```
.github/
├── profile/
│   └── README.md                    # Organization profile page
├── workflows/
│   ├── greetings.yml               # Welcome new contributors
│   ├── stale.yml                   # Stale issue/PR management
│   └── label-checker.yml           # PR label suggestions
├── ISSUE_TEMPLATE/
│   ├── bug_report.yml              # Bug report template
│   ├── feature_request.yml         # Feature request template
│   ├── question.yml                # Question template
│   └── config.yml                  # Issue config with quick links
├── DISCUSSION_TEMPLATE/
│   ├── general.yml                 # General discussions
│   ├── project-proposal.yml        # New project proposals
│   └── philosophy-erc.yml          # Philosophy & ERC discussions
├── CODE_OF_CONDUCT.md              # Community standards
├── CONTRIBUTING.md                 # Contribution guidelines
├── CONTRIBUTORS.md                 # Contributor recognition
├── FAQ.md                          # Frequently asked questions
├── FUNDING.yml                     # GitHub Sponsors configuration
├── GETTING_STARTED.md              # Newcomer onboarding
├── GLOSSARY.md                     # Terminology guide
├── GOVERNANCE.md                   # Governance model
├── IMPROVEMENTS_SUMMARY.md         # Changelog of improvements
├── LEARNING.md                     # Learning paths & education
├── LICENSE                         # CC-BY-SA 4.0
├── PINNED_REPOSITORIES.md          # Curation recommendations
├── PULL_REQUEST_TEMPLATE.md        # PR template
├── README.md                       # This file
├── REPOSITORY_TOPICS.md            # Discoverability-focused topics
├── REPOSITORY_TOPICS_ALIGNED.md    # ERC-focused topics
├── RESEARCH.md                     # Research & publications
├── SECURITY.md                     # Security policy
├── SUPPORT.md                      # Support guide
├── SUSTAINABILITY.md               # Funding & sustainability model
└── VISION_AND_ROADMAP.md           # Strategic vision (2025-2032)
```

## 📊 Documentation Stats

- **Total Documents**: 29 files
- **Community Health Files**: 5 (CONTRIBUTING, CODE_OF_CONDUCT, SECURITY, SUPPORT, CONTRIBUTORS)
- **GitHub Automation**: 3 workflows + 4 issue templates + 3 discussion templates
- **Onboarding & Learning**: 4 (GETTING_STARTED, FAQ, GLOSSARY, LEARNING)
- **Strategic & Governance**: 5 (VISION_AND_ROADMAP, GOVERNANCE, SUSTAINABILITY, RESEARCH, IMPROVEMENTS)
- **Repository Management**: 3 (TOPICS, TOPICS_ALIGNED, PINNED)

## 🌊 Our Philosophy

**Evolving Resonant Co-creationism (ERC)**: We believe the universe is a conscious, creative project we all participate in through the Meta-Principle of Infinite Love.

**Core Principles**:
- 🧘 **Consciousness-First**: Preserve awareness, not exploit attention
- 🤝 **Radical Accessibility**: Serve ALL beings equally
- 🌱 **Regenerative Economics**: Seven-year transition to community ownership
- 💎 **Sacred Development**: Code with love, wisdom, and intention
- 🔄 **Gift Economy**: Sacred reciprocity over extraction

→ Learn more: [ERC Philosophy](https://github.com/Luminous-Dynamics/evolving-resonant-cocreationism)

## 💚 Get Involved

- ⭐ **Star** repositories that interest you
- 💬 **Join** [Discussions](https://github.com/orgs/Luminous-Dynamics/discussions)
- 🐛 **Report** bugs or request features
- 🔧 **Contribute** code, docs, or design
- 💝 **Support** via [Ko-fi](https://ko-fi.com/luminousdynamics)

## 📧 Contact

- **General**: invest@luminousdynamics.org
- **Security**: invest@luminousdynamics.org with `[SECURITY]` in subject
- **GitHub**: [@Luminous-Dynamics](https://github.com/Luminous-Dynamics)

---

**Last Updated**: November 2025
**License**: CC-BY-SA 4.0

*Building a more conscious technological future, together* 💚✨
